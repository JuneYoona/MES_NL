...
{
    public class QualityInspectItemVM : ViewModelBase
    {
        #region Services
        ...
        {
            return !string.IsNullOrEmpty(EditItemCode) && !string.IsNullOrEmpty(EditQrType);
        }
        ...
        public void SearchCore()
        {
            ...
            Collections = new QualityInspectItemList(editQrType, editItemCode);
            IsBusy = false;
            IsSearch = true;
        }
        ...
        public bool CanSave()
        {
            ...
            // 필수 입력값 처리
            foreach (QualityInspectItem item in Collections.Where(u => u.State == Common.Common.EntityState.Added || u.State == Common.Common.EntityState.Modified))
            {
        ...
        public void SaveCore()
        {
            try
            {
                Collections.ToList().ForEach(u => { u.QrType = EditQrType; u.ItemCode = EditItemCode; });
                Collections.Save();
                OnSearch();
            }
            catch (Exception ex)
